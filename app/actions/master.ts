...
import { invalidateImportCache, invalidateMasterCache, withCache } from "@/lib/cache"
...
export async function listKategori() {
  return withCache("master:kategori", async () => {
...
export async function listJenis(kategoriId?: string) {
  const key = kategoriId ? `master:jenis:${kategoriId}` : "master:jenis:all"
...
export async function listSub(jenisId?: string) {
  const key = jenisId ? `master:sub:${jenisId}` : "master:sub:all"
  return withCache(key, async () => {
    const sb = await createClient()
...
export async function listUnitKerja() {
  return withCache("master:unit_kerja", async () => {
    const sb = await createClient()
...
export async function listRekening() {
  return withCache("master:rekening", async () => {
    const sb = await createClient()
...
export async function listJenisPemindahan() {
  return withCache("master:jenis_pemindahan", async () => {
    const sb = await createClient()
...
export async function listKategoriPengeluaran() {
  return withCache("master:kategori_pengeluaran", async () => {
    const sb = await createClient()
    const { data, error } = await sb
...
export async function listJenisPengeluaran(kategoriId?: string) {
  const key = kategoriId ? `master:jenis_pengeluaran:${kategoriId}` : "master:jenis_pengeluaran:all"
  return withCache(key, async () => {
    const sb = await createClient()
    let q = sb
