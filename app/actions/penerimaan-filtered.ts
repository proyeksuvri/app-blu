...
export async function getPenerimaanFiltered(
  ...
  // ── Query 1: Data utama (dengan batching agar tidak terpotong limit 1000 Supabase) ──
  ...
  // ── Query 2: Periode sebelumnya (hanya jumlah, batching) ──
  let prevTotal = 0
  {
    let offset = 0
    const baseQ = sb
    ...
    while (true) {
      const { data: batch, error } = await baseQ.range(offset, offset + BATCH - 1)
      if (error || !batch || batch.length === 0) break
      prevTotal += batch.reduce((s, r) => s + Number(r.jumlah), 0)
      if (batch.length < BATCH) break
      offset += BATCH
    }
  }

  // ── Query 3: Tahun lalu (hanya jumlah, batching) ──
  let prevYearTotal = 0
  {
    let offset = 0
    const baseQ = sb
    ...
    while (true) {
      const { data: batch, error } = await baseQ.range(offset, offset + BATCH - 1)
      if (error || !batch || batch.length === 0) break
      prevYearTotal += batch.reduce((s, r) => s + Number(r.jumlah), 0)
      if (batch.length < BATCH) break
      offset += BATCH
    }
  }
  ...
  // Unique active days (based on tanggal_terima)
