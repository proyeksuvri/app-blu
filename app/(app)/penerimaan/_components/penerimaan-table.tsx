...
export function PenerimaanTable({ data, isAdmin, sort, order, totalDraft, totalDeletable, subOptions = [], filter }: {
  ...
  async function handleDownload() {
    ...
    // 10 kolom: tanggal_transaksi, kode_kategori, kode_jenis, kode_sub, kode_unit,
    //           kode_rekening, kode_metode, jumlah, nomor_bukti, uraian
    ws["!cols"] = [16, 16, 16, 16, 14, 16, 18, 14, 18, 30].map((wch) => ({ wch }))
  ...
  return (
    <>
      ...
      <div className="flex justify-end gap-2">
        ...
        <DownloadDetailButton filter={filter} sort={sort} order={order} />
