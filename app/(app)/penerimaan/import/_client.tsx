    ...
    reader.onload = async (e) => {
      try {
        ...
        const importRows: ImportRow[] = rows.map((r, i) => ({
          baris: i + 2,
          tanggal_terima: formatDate(r["tanggal_transaksi"] ?? r["tanggal_terima"] ?? r["tanggal"] ?? r["tgl"]),
          ...
          jumlah: parseAmount(r["jumlah"] ?? r["nominal"] ?? r["kredit"]),
          nomor_referensi: (r["nomor_bukti"] ?? r["nomor_referensi"] ?? r["no_referensi"] ?? r["ref"]) ? String(r["nomor_bukti"] ?? r["nomor_referensi"] ?? r["no_referensi"] ?? r["ref"]) : undefined,
