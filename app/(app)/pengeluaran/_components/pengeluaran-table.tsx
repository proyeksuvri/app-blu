...
export function PengeluaranTable({ data, isAdmin, sort, order, totalDraft, totalDeletable, filter }: {
  data: Row[]
  isAdmin: boolean
  sort: SortKey
  order: "asc" | "desc"
  ...
  function handleBulkVerify() {
    startTransition(async () => {
      const result = await bulkVerifyPengeluaran(draftIds.filter((id) => selected.has(id)))
  ...
  function handleBulkDelete() {
    ...
    startTransition(async () => {
      const result = await bulkDeletePengeluaran(selectedArray)
      if (!result.ok) { toast.error(result.pesan); return }
