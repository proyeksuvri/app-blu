...
import { PengeluaranActions } from "./_actions"
import { format } from "date-fns"
import { id } from "date-fns/locale"
import Link from "next/link"
import { ArrowLeft } from "lucide-react"

export default async function PengeluaranDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const profile = await getCurrentProfile()
  if (!profile) redirect("/")

  const { id: pengeluaranId } = await params
  const data = await getPengeluaran(pengeluaranId)
  if (!data) notFound()
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      <div className="flex items-center gap-3">
        <Link href="/pengeluaran" className="text-muted-foreground hover:text-foreground/70 transition-colors">
      ...
      <div className="rounded-xl border border-border divide-y divide-border/50">
        <Row label="Status">
          <StatusBadge status={data.status as "draft" | "verified" | "void"} />
        </Row>
        <Row label="Tanggal Transaksi">{fmt(data.tanggal)}</Row>
