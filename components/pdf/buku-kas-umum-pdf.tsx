...
/** Mengembalikan hari terakhir bulan dari string tanggal (YYYY-MM-DD) */
const tglAkhirBulan = (s: string) => {
  const [year, month] = s.split("-").map(Number)
  // new Date(year, month, 0) → hari terakhir bulan 'month' di tahun 'year'
  ...
  return (
    ...
    >
      <Page size="A4" orientation="landscape" style={s.page}>
        ...
        {/* ── Tanda Tangan ── */}
        <View style={[s.ttdSection, { alignItems: "flex-end" }]}>
          <View style={s.ttdBox}>
            <Text style={{ fontSize: 7.5, color: C.textMuted, marginBottom: 6, textAlign: "center" }}>
              Palopo, {tglAkhirBulan(filter.tglAkhir)}
            </Text>
            <Text style={[s.ttdLabel, { textAlign: "center" }]}>Dibuat oleh,</Text>
            <Text style={[s.ttdLabel, { textAlign: "center" }]}>Bendahara Penerimaan</Text>
            <View style={s.ttdGaris} />
            <Text style={[s.ttdNama, { textAlign: "center" }]}>SUVRI ABDILLAH, S.SOS</Text>
          </View>
        </View>
