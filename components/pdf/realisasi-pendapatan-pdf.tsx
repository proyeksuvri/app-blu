...
import type { RealisasiPendapatanRow } from "@/app/actions/laporan"
...
interface Props {
  tglAwal: string
  tglAkhir: string
  rows: RealisasiPendapatanRow[]
}

export function RealisasiPendapatanPDF({
  tglAwal,
  tglAkhir,
  rows,
}: Props) {
    ...
    <Document author="BLU UIN Palopo" title={`Realisasi Pendapatan ${periode}`}>
      <Page size="A4" orientation="portrait" style={s.page}>
        ...
        <View style={s.table}>
          ...
          {rows.map((row, i) => (
            <View key={i} style={[s.tr, i % 2 !== 0 ? s.trAlt : {}]} wrap={false}>
              <Text style={[s.td, s.colNo, { textAlign: "center" }]}>{i + 1}</Text>
              <Text style={[s.td, s.colAkun, { textAlign: "center" }]}>{row.akun_pendapatan}</Text>
              <Text style={[s.td, s.colJenis, { textAlign: "left" }]}>{row.nama_jenis}</Text>
              <Text style={[s.td, s.colJumlah]}>{rupiah(row.jumlah)}</Text>
            </View>
          ))}

          <View style={s.trTotal} wrap={false}>
            <Text style={[s.tdTotal, s.colTotalLabel]}>TOTAL PENDAPATAN</Text>
